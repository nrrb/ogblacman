...
Implementation is in progress. The current build includes a responsive application shell, typed provisional content, static release routes, metadata, a persistent custom OGAmp DnB playlist, the complete Tree Hugging game loop, the client-supplied Black Buddha figure, automated tests, and a Surge preview workflow.
...
- Provide **OGAmp**, a persistent custom audio player that survives route navigation.
...
- Web Audio API for OGAmp's audio-reactive spectrum visualization
