...
const releaseArt = secondSlide.locator('.release-spotlight__art')
if (!(await releaseArt.isVisible())) throw new Error('Telephone cover art is missing')
if (await releaseArt.getAttribute('src') !== '/assets/releases/telephone-cover.png') throw new Error('Telephone cover art source is incorrect')
...
const releaseArtBox = await secondSlide.locator('.release-spotlight__art-link').boundingBox()
const releaseDetailsBox = await secondSlide.locator('.release-spotlight__details').boundingBox()
const webampBox = await webamp.boundingBox()
if (!releaseArtBox || !releaseDetailsBox || !webampBox) throw new Error('Hot release layout is not measurable')
if (releaseArtBox.width > 120) throw new Error('Telephone cover art should remain a compact thumbnail on mobile')
if (releaseArtBox.x + releaseArtBox.width >= releaseDetailsBox.x || releaseArtBox.x + releaseArtBox.width >= webampBox.x) {
  throw new Error('Telephone cover art should sit beside the release title and player')
}
